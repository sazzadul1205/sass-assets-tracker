...
const EditCategoryModal = ({ Refetch, selectedCategory, setSelectedCategory }) => {
  ...
  useEffect(() => {
    if (selectedCategory) {
      reset({
        ...
        depreciation_rate: selectedCategory?.depreciation_rate || 0,
        useful_life_years: selectedCategory?.useful_life_years || "",
        description: selectedCategory?.description || "",
      });
      setSelectedColor(selectedCategory?.icon_color || "#ffffff");
      setIconImage(null);
    }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (!selectedCategory?._id) {
        setError("Selected category data is missing. Please try again.");
        throw new Error("Selected category data is missing. Please try again.");
      }

      // Use existing icon if no new image uploaded
      let uploadedImageUrl = selectedCategory?.icon_url || null;
      ...
      const response = await axiosPublic.put(
        `/AssetCategories/${selectedCategory._id}`,
        payload
      );
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        setTimeout(() => setClearImageInput(false), 100);
      ...
      console.error(err);
      setError(err?.message || "Something went wrong while updating category");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="flex gap-5 items-center">
              ...
              >
                <SharedImageInputCircular
                  ...
                  defaultImage={selectedCategory?.icon_url || null}
                />
              </div>
